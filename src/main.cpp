...
#if defined(NDEBUG)
# error "Czechcrowncoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x8a291223f900cb1535429b4422d08233df629ff7f2c0e83a26fb79422e3a62dd");
static CBigNum bnProofOfWorkLimit(~uint256(1)/*0) >> 20*/); // Czechcrowncoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Czechcrowncoin Signed Message:\n";
...
{
    // Czechcrowncoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Czechcrowncoin: 3.5 days
static int64 nTargetSpacing = 2.5 * 60; // Czechcrowncoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Czechcrowncoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Czechcrowncoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Czechcrowncoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x000c7102eb5cf901d2437f06739957da5e07a398712095c8d6471f0e0bcc06eb");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Sun, 29 Jun 2014 00:00:00 GMT";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1404000000;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 393337795;

        if (fTestNet)
        {
            block.nTime    = 1317800000;
            block.nNonce   = 386977496;
        }
        ...
        assert(block.hashMerkleRoot == uint256("0xf330b28041cae0049eed5f4f52d6651fdc0122282e1610983f444c44bbf35d51"));
        //block.print();
...
unsigned char pchMessageStart[4] = { 0xc7, 0xec, 0xdb, 0xd1 }; // Czechcrowncoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("CzechcrowncoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("CzechcrowncoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static CzechcrowncoinMiner(CWallet *pwallet)
{
    printf("CzechcrowncoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("czechcrowncoin-miner");
    ...
    try { loop {
        ...
        printf("Running CzechcrowncoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("CzechcrowncoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&CzechcrowncoinMiner, pwallet));
}
